...
void Application::handleEvent(sf::Event event, sf::RenderWindow& window)
{
    ...
    case sf::Event::KeyPressed:
        ...
        case sf::Keyboard::A: // A is for AZERTY keyboards
        case sf::Keyboard::Q: // Q is for QWERTY and QWERTZ keyboards
          getStats().focusOnRelativeGraph(-1);
          break;
...
void Application::drawControls(sf::RenderWindow& target) {
	...
	auto const FONT_SIZE = 20;
	drawTitle(target, sf::Color::White, LEGEND_MARGIN + 40, lastLegendY - 120, FONT_SIZE);
  drawTime(target, sf::Color::White, 100, lastLegendY - 80, 20);
  drawFPS(target, sf::Color::White, 100, lastLegendY - 40, 20);
	lastLegendY += FONT_SIZE + 4;
...
void Application::drawTime(sf::RenderWindow& target
                 , sf::Color color
                 , size_t xcoord
                 , size_t ycoord
                 , size_t font_size
...
{
  std::string textTime = std::to_string(floor(((getAppEnv().getTime()).asSeconds())*100)/100);
  std::string textTime2 = "Time : " + textTime.replace(textTime.find(".")+2, 5, "");
  auto time = sf::Text(textTime2, getAppFont(), font_size);
  time.setPosition(xcoord, ycoord);
...
void Application::drawFPS(sf::RenderWindow& target
                 , sf::Color color
                 , size_t xcoord
                 , size_t ycoord
                 , size_t font_size
...
{
  std::string textFPS = std::to_string(fps);
  auto valueFPS = sf::Text(textFPS, getAppFont(), font_size);
  valueFPS.setPosition(xcoord, ycoord);
  
  #if SFML_VERSION_MAJOR >= 2 && SFML_VERSION_MINOR >= 4
	valueFPS.setFillColor(color);
#else
	valueFPS.setColor(color);
#endif
	target.draw(valueFPS);
}
